...
export const Admin: React.FC<AdminProps> = ({ cars, onAddCar, onUpdateCar, onDeleteCar }) => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'fleet' | 'bookings'>('dashboard');
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (email === 'admin@elitedrive.by' && password === 'admin') {
      setIsAuthenticated(true);
    } else {
      alert('Неверный логин или пароль (попробуйте admin@elitedrive.by / admin)');
    }
  };
  ...
  const handleSaveCar = (e: React.FormEvent) => {
    e.preventDefault();
    if (currentCar.id && currentCar.name) {
           ...
           <div className="animate-fade-in-up">
             ...
             <div className="bg-dark-900 rounded-xl border border-white/5 overflow-hidden">
                <div className="p-8 text-center text-gray-500">
                    ...
                    <p className="text-sm mt-2">В демо-версии заявки не сохраняются в базу данных.</p>
                </div>
             </div>
           </div>
        )}
          ...
          <div className="relative bg-dark-800 rounded-2xl w-full md:max-w-2xl border border-white/10 my-8">
            <div className="p-6 border-b border-white/10 flex justify-between items-center sticky top-0 bg-dark-800 rounded-t-2xl z-10">
              <h3 className="text-xl font-serif text-white pr-8">
                {currentCar.id && cars.find(c => c.id === currentCar.id) ? 'Редактировать' : 'Добавить'} автомобиль
              </h3>
            ...
            <form onSubmit={handleSaveCar} className="p-6 space-y-6">
              ...
              <div>
                 <label className="block text-xs uppercase text-gray-500 mb-2">Ссылка на изображение</label>
                  <input 
                    required
                    type="text" 
